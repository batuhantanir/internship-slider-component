        ...
        <div className='grid md:grid-cols-2 items-center justify-center px-10 py-6 gap-8   text-center z-10'>
          {mainImageOpen && (
              <img
                ...
                alt="Main"
                className={`${changePosition && "order-2"} rounded-xl sm:rounded-[40px] w-3/4 max-w-4xl h-full max-h-[30rem] md:p-6 transition-all duration-500 ease-in-out hover:scale-105`}
              />
          )}
          <div className={`flex flex-col items-center ${changePosition && "order-1"}`}>
            {MainTextOpen && (
              <h1 className={`text-2xl md:text-4xl xl:text-7xl order-1  font-bold ${mainText ? 'block' : 'hidden'}`} style={{ color: mainTextColour ? mainTextColour : "black" }} >
                {mainText}
              </h1>
            )}
            {subTextOpen && (
              <p className={`text-lg md:text-2xl xl:text-5xl my-3 order-3 md:order-2 ${subText ? 'block' : 'hidden'}`} style={{ color: subTextColour ? subTextColour : "black" }}>
                {subText}
              </p>
            )}
            {buttonOpen && (
              <Link
                href={buttonLink}
                className={`text-sm  xl:text-xl py-2 px-4 rounded-full order-2 md:order-3 w-fit mt-2 inline-block font-semibold  transition-all duration-500 ease-in-out origin-center hover:rotate-6`}
